...
.. seealso::
   For more detailed information about the **Surveys** app, refer to the :doc:`survey essentials
...
.. note::
   The :guilabel:`Do not digitize` option for :guilabel:`Résumé digitization (OCR)` at first may
   seem like a redundancy. It seems to be the same as disabling the :guilabel:`Résumé digitization
   (OCR)` option.

   When the :guilabel:`Résumé digitization (OCR)` option is enabled, a module is installed so that
   résumés can be scanned. Disabling this option uninstalls the module.

   If at some point, there is a desire to temporarily stop digitizing résumés, select the
   :guilabel:`Do not digitize` option. The reason this option is available is so that the module is
   not uninstalled, allowing for digitization to be enabled in the future by selecting one of the
   other two options.
